...
- **Fix #62**: `getLastTagForPackage()`, `getLastStableTagForPackage()` y `getLatestRemoteStableVersion()` dejan de confiar en el orden de `--sort=-v:refname` y ordenan/seleccionan por **semver real en JS** (`semver.rcompare` sobre la versión extraída del tag, descartando tags cuya versión no parsea). El baseline del escaneo pasa a ser siempre el tag de mayor versión semver del package.
