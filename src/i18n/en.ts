import type { Messages } from "./types.js";

export const en: Messages = {
  wizard: {
    noPackages: "No valid packages found in this project.",
    bye: "Bye!",
    error: "An error occurred.",
  },
  workspace: {
    undeclaredPackages: (filePath) => `${filePath} does not declare "packages", so this repository is treated as a single package. If you expected a monorepo, check that key in the file.`,
    invalidPackageJson: (filePath) => `Could not read the package.json at ${filePath}. Skipping that package.`,
  },
  schemaError: {
    header: (filePath) => `Invalid content in ${filePath}:`,
    rootField: "(root)",
  },
  menu: {
    question: "What do you want to do?",
    createRelease: "New version",
    createReleaseHint: "Scan commits, bump version, create git tag and CHANGELOG",
    githubRelease: "GitHub release from existing tag",
    githubReleaseHint: "Publish an already-created local tag to GitHub Releases",
    exit: "Exit",
  },
  githubRelease: {
    noConfig: "GitHub release skipped: github.createRelease is not enabled in tagman.config.json.",
    noToken: "GitHub release skipped: no token found. Set GITHUB_TOKEN env var or add 'github_token=TOKEN' to ~/.npmrc.",
    noRemote: "GitHub release skipped: could not detect owner/repo from the origin remote.",
    noTags: "No local git tags found. Create a release first.",
    loadingTags: "Loading local tags...",
    tagsLoaded: (count) => `Found ${count} local tag(s).`,
    selectTags: "Select the tags to publish as GitHub Releases:",
    selectPackage: "Choose a package to add a release for:",
    selectTagVersion: (pkg) => `Choose which version of ${pkg} to publish:`,
    doneSelectingTags: "Done — publish selected releases",
    noTagsSelectedYet: "You haven't selected any tags yet.",
    alreadySelected: "already selected",
    cancelled: "Operation cancelled.",
    creating: "Creating GitHub Releases...",
    done: (count) => `${count} GitHub Release(s) created successfully.`,
    failed: (tag, msg) => `GitHub release failed for tag ${tag}: ${msg}`,
    deselectTag: "Remove from selection",
    abortedNoToken: "Cannot continue without a GitHub token. Set GITHUB_TOKEN or retry the login.",
  },
  checkpoint: {
    uncommittedWarning: "Warning:",
    uncommittedQuestion: "You have uncommitted local changes. Do you want to continue anyway?",
    interruptedRelease: (step) => `An interrupted release was found at phase "${step}". Do you want to resume it?`,
    resumeQuestion: "An interrupted release was found. Do you want to resume it?",
    rollbackQuestion: "Do you want to revert the local changes to package.json and CHANGELOG.md that tagman managed to make?",
    rollingBack: "Reverting files to pre-release state...",
    rollbackDone: "Rollback complete. Files reverted successfully.",
    rollbackGitWarn: "Could not revert the git commit. Please verify manually with git log.",
    cancelled: "Operation cancelled.",
    fetchingRemote: "Checking remote for new commits...",
    behindRemote: (count) => `Your branch is ${count} commit(s) behind origin.`,
    behindQuestion: "Do you want to continue without pulling?",
    behindBlocked: "Pull the latest changes before releasing.",
    remoteUnverified: (reason) => `Could not verify remote status (${reason}). Continuing without sync verification.`,
    remoteUnverifiedTimeout: "timed out",
    remoteUnverifiedAuth: "no credentials for remote",
    remoteUnverifiedError: "remote unreachable",
  },
  scan: {
    scanning: "Scanning packages for new commits...",
    scanDone: (total, found) => `Scanned ${total} packages. Found ${found} with pending changes.`,
    nothingToRelease: "No new commits found in any package. Nothing to release.",
    nothingToReleaseMenu: "No pending commits found. What do you want to do?",
    selectPackages: "Step 1: Select packages to release",
    noMatchingPackages: (filter) => `None of the specified packages (${filter}) have pending commits.`,
    selectCommits: (pkg) => `Step 2: Commits for`,
    selectBump: (pkg, current) => `Step 3: Version increment for`,
    goBack: "Go back to commit selection",
    goBackToPackages: "Go back to package selection",
    selectExtraCommits: (pkg) => `Step 2b: Additional commits (optional) for`,
    selectAll: "select all",
    deselectAll: "deselect all",
    showDetails: "show date & author",
    hideDetails: "hide date & author",
    customVersion: (pkg) => `Enter the exact SemVer version for ${pkg}:`,
    customVersionError: "Error: must be a valid SemVer version (e.g. 1.2.3)",
    cascadeQuestion: (dep, consumer) => `Notice: ${dep} is a dependency of ${consumer}. Do you want to also version ${consumer} to update its reference?`,
    cancelled: "Operation cancelled.",
    preRelease: "Pre-release",
    preReleaseHint: "Create an alpha/beta/rc version",
    hotfix: "Hotfix",
    hotfixHint: "Create a hotfix for a specific version",
    hotfixIncrement: (version) => `Increment hotfix  (${version})`,
    selectPreReleaseType: (pkg) => `Step 3a: Pre-release base bump for ${pkg}`,
    preReleaseIncrement: (version) => `Increment counter  (${version})`,
    preReleaseGraduate: (version) => `Graduate to stable  (${version})`,
    prepatch: (version) => `prepatch  (${version})`,
    preminor: (version) => `preminor  (${version})`,
    premajor: (version) => `premajor  (${version})`,
    selectChannel: "Step 3b: Pre-release channel",
    channelCustom: "Custom channel name",
    channelCustomInput: "Enter the channel name (e.g. alpha, beta, rc):",
    channelBranchHint: "current branch",
    channelNextStepHint: "the channel is chosen in the next step",
    channelSanitized: (original, sanitized) =>
      `Channel name "${original}" contains invalid semver characters and was sanitized to "${sanitized}".`,
    trailingCommitsWarning: (count) => `Warning: ${count} commit(s) newer than your selection will be included in the tag's code but not in the changelog, and won't appear in the next scan.`,
    trailingCommitsQuestion: "How do you want to handle these commits?",
    trailingReorder: "Reorder — tag at the selected code state (safe, local-only)",
    trailingAddAll: "Add them to this release",
    trailingContinue: "Continue anyway (included in code, excluded from changelog)",
    trailingGoBack: "Go back to commit selection",
    graduationFound: (count) => `Found ${count} package(s) eligible for graduation to stable.`,
    graduationHint: (version) => `no new commits — graduate ${version} to stable`,
    zeroCommitsGraduate: (pkg, version) => `No commits selected for ${pkg} (${version}). Graduate to stable instead?`,
    graduationBlockedPushed: "Cannot graduate at this code state: some commits since the pre-release tag have already been pushed.",
    graduationBlockedReorder: "Cannot graduate: a commit reorder is already in progress for another package in this run.",
    extraOnlyHint: "no new commits — shared commits available",
    noCommitsHint: "no changes detected",
    graduationCheckingRemote: "Checking remote for conflicting tags...",
    graduationConflictWarning: (target, remote) =>
      `Version ${target} is already taken on the remote (latest stable: ${remote}).`,
    graduationConflictQuestion: (suggested) =>
      `Upgrade graduation target to ${suggested} instead?`,
    navUpDown: "navigate",
    navSelect: "select",
    navConfirm: "confirm",
    selectExtraCommitsCount: (count, lastTag) =>
      lastTag ? `${count} commit(s) · since ${lastTag}` : `${count} commit(s)`,
    cascadeGoBack: "Go back to step 3",
    cascadeYes: "Yes",
    cascadeNo: "No",
    progress: (current, total) => `[Package ${current} of ${total}]`,
    removePackages: "Remove package(s) from selection",
    removePackagesTitle: "Deselect the packages you want to remove from the release:",
    removePackagesEmpty: "All packages were removed. Going back to scan.",
  },
  tagMessages: {
    createTagQuestion: (pkg, version) => `Create a Git tag for ${pkg}@${version}?`,
    createTagYes: "Yes, create tag",
    createTagNo: "No, skip tag",
    goBackToPrevious: "Go back to previous package",
    autoGenLabel: "Auto-generated message for",
    actionSelect: "Which message do you want to use for the tag?",
    useAuto: "Use the auto-generated message",
    appendText: "Append additional text to the auto-generated message",
    writeCustom: "Write a completely new message",
    goBack: "Go back to package selection",
    goBackToAction: "Go back to message selection",
    appendInput: "Additional text:",
    insertPosition: "Where do you want to insert this text?",
    insertBefore: "Before the commit list",
    insertAfter: "At the end of the message",
    customInput: "New message for the tag:",
    cancelToBack: "(Ctrl+C to go back)",
    cancelled: "Operation cancelled.",
    noTagsWarning: "No packages have a tag configured. Review your tag options before continuing.",
    tagSummaryTitle: "Tag summary",
    tagSummaryCreate: "create tag",
    tagSummarySkip: "skip tag",
    graduationActionSelect: "Which message do you want to use for the graduation tag?",
    useGraduationAuto: "Use the auto-generated graduation message (with from/to context)",
    includeChangelog: "Include full pre-release channel CHANGELOG",
    changelogEmpty: "No pre-release entries found in CHANGELOG. Please choose another option.",
    changelogPreview: "Pre-release channel changelog preview",
  },
  draft: {
    found: (date) => `A draft was found from ${date}.`,
    resumeQuestion: "Do you want to resume it?",
    resume: "Resume draft",
    discard: "Discard and start fresh",
    summaryTitle: "Planned changes summary",
    actionQuestion: "What do you want to do?",
    proceed: "Proceed with the tag creation",
    save: "Save as draft and exit",
    goBack: "Go back to edit selections",
    saved: "Draft saved. You can resume it next time you run tagman.",
    detailsShow: "show commits",
    detailsHide: "hide commits",
    staleTitle: "Draft verification",
    blockedReorderStale: "The draft includes a commit reorder, but the repository HEAD changed since it was saved. The reorder is no longer safe to apply: it can only be discarded.",
    blockedVersionChanged: (pkg) => `The version of "${pkg}" changed since the draft was saved; the planned bump is no longer valid.`,
    warnNewCommits: "There are new commits since the draft was saved that are not part of the plan.",
    warnBranchDiff: (saved, current) => `The draft was saved on branch "${saved}"; you are now on "${current}".`,
    warnUnverifiable: "This draft's validity cannot be verified (it was saved by an older version of tagman).",
  },
  execute: {
    confirmProceed: "All set. Proceed with writing, commits and tags?",
    confirmYes: "Yes, proceed",
    confirmNo: "No, cancel",
    goBack: "Go back to review tag messages",
    cancelled: "Cancelled by user.",
    writing: "Writing changes...",
    writingError: "Error updating files.",
    writingDone: "Files updated.",
    committing: "Creating git commit & tags...",
    commitDone: "Git configured.",
    tagsSpinnerError: "Error creating tags.",
    pushSpinnerError: "Push failed.",
    tagsError: (msg) => `Tag creation failed: ${msg}`,
    reverting: "Reverting commit and already-created tags...",
    revertedGit: "Git changes were reverted. Files remain modified on disk.",
    pushQuestion: "Push commits and tags to remote now?",
    pushing: "Pushing to remote...",
    pushDone: "Push complete.",
    pushError: (msg) => `git push failed: ${msg}`,
    pushFallback: "You can do it manually: git push --follow-tags",
    pushHttpsLoginPrompt: "The remote uses HTTPS and no credentials are available. Sign in to GitHub to push?",
    pushRetrying: "Retrying push with the GitHub session...",
    githubCreating: "Creating GitHub Releases...",
    githubDone: (count) => `GitHub Releases created: ${count}`,
    githubNoToken: "GitHub release skipped: no token found (set GITHUB_TOKEN env var or add it to ~/.npmrc)",
    githubNoRemote: "GitHub release skipped: could not detect owner/repo from remote.",
    githubFailed: (pkg, msg) => `GitHub release failed for ${pkg}: ${msg}`,
    npmPublishing: "Publishing packages to npm...",
    npmDone: "Publishing complete.",
    npmPublished: (pkg, version) => `  ✓ ${pkg}@${version} published`,
    npmFailed: (pkg, msg) => `  npm publish failed for ${pkg}: ${msg}`,
    githubTokenInsecure: (path) => `[security] Insecure permissions for ${path}. It should only be readable by the owner.`,
    githubTokenProjectExposure: "[security] github_token found in project-level .npmrc. This is insecure and no longer supported. Please use GITHUB_TOKEN env var or global ~/.npmrc.",
    githubTokenConfigExposure: "[security] github.token found in tagman.config.json. This is insecure and no longer supported. Please use GITHUB_TOKEN env var or global ~/.npmrc.",
    githubSkippedPrerelease: "GitHub Releases skipped: pre-release versions are not published automatically. Use `tagman github-release` to publish them manually.",
    done: "Release complete! Versions generated successfully.",
    dryRunHeader: "--- DRY RUN: changes that would be applied ---",
    dryRunDone: "Dry run complete. No changes were made.",
    reorderLifting: "Reordering commits around release...",
    reorderDone: "Commits reordered successfully.",
    reorderFailed: (msg) => `Commit reorder failed: ${msg}. Reverting to original state.`,
    githubDeviceLoginPrompt: "No GitHub token found. Do you want to do a one-time login via browser to create the release? (The token will not be saved)",
    githubDeviceLoginInstructions: (url, code) => `Please open ${url} and enter the code: ${code}`,
    githubDeviceLoginSuccess: "GitHub login successful.",
    githubDeviceLoginFailed: (msg) => `GitHub login failed: ${msg}`,
  },
};
